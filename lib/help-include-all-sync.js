    ...
    files.forEach(function (file) {
      ...
      // So we'll go ahead and add a key for it in our module map.
        ...
        // this module into memory using `require()`
          ...
          if (!options.useFilenameAsKey) {

            if (module.globalId && options.useGlobalIdForKeyName) { keyName = module.globalId; }
            else if (module.identity) { keyName = module.identity; }
            else {
              if (options.replaceExpr) {
                keyName = keyName.replace(options.replaceExpr, options.replaceVal);
              }
              if (path.sep === '\\') {
